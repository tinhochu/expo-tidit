import { Skia } from '@shopify/react-native-skia'

import JustListedTemplateOne from './templates/just-listed/templateOne'
import JustSoldTemplateOne from './templates/just-sold/templateOne'

interface TemplateRendererProps {
  postType: 'JUST_SOLD' | 'JUST_LISTED' | 'JUST_RENTED' | 'OPEN_HOUSE' | 'UNDER_CONTRACT' | 'BACK_ON_MARKET' | 'LOADING'
  data: any
  template: string
  canvas: any
  userPrefs: any
}

interface TemplateComponent {
  label: string
  value: string
  component: React.ComponentType<{ data: any; postType: string; template: string; canvas: any; userPrefs: any }>
}

export const sqftIcon = (color: string) =>
  Skia.SVG.MakeFromString(
    `<svg width="16px" height="16px" viewBox="0 0 20 20" version="1.1" xmlns="http://www.w3.org/2000/svg">
      <g id="layer1">
        <path d="M 0 0 L 0 20 L 20 20 L 0 0 z M 1 2.4160156 L 17.583984 19 L 15 19 L 15 18 L 14 18 L 14 19 L 12.998047 19 L 12.998047 18 L 12 18 L 12 19 L 11 19 L 11 18 L 10 18 L 10 19 L 9 19 L 9 18 L 8 18 L 8 19 L 7 19 L 7 18 L 6 18 L 6 19 L 5 19 L 5 18 L 4 18 L 4 19 L 3 19 L 3 18 L 2 18 L 2 19 L 1 19 L 1 2.4160156 z M 4 9.6582031 L 4 16 L 10.34375 16 L 4 9.6582031 z M 5 12.072266 L 7.9277344 15 L 5 15 L 5 12.072266 z " style="fill:${color}; fill-opacity:1; stroke:none; stroke-width:0px;"/>
      </g>
    </svg>`
  )

export const bedIcon = (color: string) =>
  Skia.SVG.MakeFromString(
    `<svg width="16px" height="16px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M3 5V19M3 16H21M21 19V13.2C21 12.0799 21 11.5198 20.782 11.092C20.5903 10.7157 20.2843 10.4097 19.908 10.218C19.4802 10 18.9201 10 17.8 10H11V15.7273M7 12H7.01M8 12C8 12.5523 7.55228 13 7 13C6.44772 13 6 12.5523 6 12C6 11.4477 6.44772 11 7 11C7.55228 11 8 11.4477 8 12Z" stroke="${color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>`
  )

export const bathIcon = (color: string) =>
  Skia.SVG.MakeFromString(
    `<svg width="16px" height="16px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path fill-rule="evenodd" clip-rule="evenodd" d="M3 5.00611L3.00018 4.99962C3.01218 4.63969 3.10892 4.26822 3.26943 3.94721C3.39761 3.69084 3.57419 3.46489 3.82033 3.3008C4.06021 3.14088 4.42499 3 5 3C5.57502 3 5.9398 3.14088 6.17967 3.3008C6.48866 3.50679 6.69219 3.8208 6.81777 4.14477C6.22162 4.30128 5.72626 4.57709 5.32555 4.93481C4.70429 5.48942 4.3823 6.17607 4.21313 6.72163C4.02059 7.34253 4.19257 7.9476 4.56764 8.37037C4.92761 8.77611 5.45463 9 6 9H10C10.5454 9 11.0724 8.77611 11.4324 8.37037C11.8074 7.9476 11.9794 7.34253 11.7869 6.72163C11.6177 6.17607 11.2957 5.48942 10.6745 4.93481C10.2084 4.51876 9.61431 4.2135 8.88252 4.07789C8.81351 3.77563 8.70182 3.41757 8.51943 3.05279C8.27261 2.55916 7.88669 2.03511 7.28908 1.6367C6.68521 1.23412 5.92499 1 5 1C4.07502 1 3.3148 1.23412 2.71092 1.6367C2.11331 2.03511 1.72739 2.55916 1.48057 3.05279C1.23572 3.5425 1.11827 4.02011 1.06048 4.36685C1.02595 4.57403 1.00127 4.78516 1.00001 4.99552L1 13C1.00011 13.0906 1.00205 13.1801 1.00864 13.3416C1.01708 13.5482 1.0338 13.8399 1.067 14.1886C1.13297 14.8812 1.26604 15.8212 1.53848 16.7747C1.80759 17.7166 2.23337 18.7464 2.92824 19.557C3.64853 20.3974 4.66199 21 6 21L6 22C6 22.5523 6.44772 23 7 23C7.55229 23 8 22.5523 8 22V21H16V22C16 22.5523 16.4477 23 17 23C17.5523 23 18 22.5523 18 22L18 21C20.0879 21 21.3006 19.566 21.9601 18.1431C22.6171 16.7257 22.8534 15.0872 22.9426 14.0842C23.048 12.8984 22.0873 12 21 12H3V5.00611ZM3.0581 14C3.11716 14.6197 3.23408 15.4292 3.46152 16.2253C3.69241 17.0334 4.01663 17.7536 4.44676 18.2555C4.85147 18.7276 5.33801 19 6 19H18C18.9628 19 19.6375 18.3981 20.1456 17.302C20.6394 16.2366 20.8536 14.926 20.9419 14H3.0581ZM9.34254 6.42679C9.52126 6.58633 9.65602 6.78564 9.75612 7H6.24388C6.34398 6.78564 6.47874 6.58633 6.65746 6.42679C6.90165 6.2088 7.29876 6 8 6C8.70124 6 9.09836 6.2088 9.34254 6.42679Z" fill="${color}"/>
      </svg>`
  )

function getTemplates(postType: string): TemplateComponent[] {
  switch (postType) {
    case 'JUST_SOLD':
      return [
        {
          label: 'Template 1',
          value: '1',
          component: JustSoldTemplateOne,
        },
      ]
    case 'JUST_LISTED':
      return [
        {
          label: 'Template 1',
          value: '1',
          component: JustListedTemplateOne,
        },
      ]
    case 'LOADING':
      return []
    default:
      return []
  }
}

function TemplateRenderer({ postType, template, data, canvas, userPrefs }: TemplateRendererProps) {
  const Template = getTemplates(postType)?.find((t) => t.value === template)?.component
  return Template ? (
    <Template data={data} postType={postType} template={template} canvas={canvas} userPrefs={userPrefs} />
  ) : null
}

export { TemplateRenderer, getTemplates }
